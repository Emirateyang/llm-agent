package com.llmagent.embedding.siliconflow.client;
...
import com.llmagent.embedding.siliconflow.api.SiliconflowApi;
...
public class DefaultEmbeddingClient extends EmbeddingClient {

    private static final Logger log = LoggerFactory.getLogger(DefaultEmbeddingClient.class);

    public final String baseUrl;
    private final OkHttpClient okHttpClient;
    private final SiliconflowApi siliconflowApi;

    public DefaultEmbeddingClient(String apiKey) {
        this(new Builder().apiKey(apiKey));
    }

    public DefaultEmbeddingClient(Builder serviceBuilder) {
        ...
        this.siliconflowApi = retrofitBuilder.build().create(SiliconflowApi.class);
    }

    @Override
    public SyncOrAsync<EmbeddingResponse> embedding(EmbeddingRequest request) {
        return new RequestExecutor<>(siliconflowApi.embeddings(request), r -> r);
    }
    ...
    public static class Builder extends EmbeddingClient.Builder<DefaultEmbeddingClient, Builder> {
        public DefaultEmbeddingClient build() {
            return new DefaultEmbeddingClient(this);
        }
    }
