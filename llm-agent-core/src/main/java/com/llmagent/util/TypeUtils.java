...
public class TypeUtils {
    ...
    private static String getExemplarDeclaration(List<Type> typeChain, String forType) {
        List<Type> rawTypesOnly = typeChain.stream().filter(type -> !(type instanceof WildcardType || type instanceof TypeVariable)).toList();
        return rawTypesOnly.stream().map(type -> TypeUtils.getRawClass(type).getSimpleName()).collect(Collectors.joining("<")) + "<" + forType +
                ">".repeat(rawTypesOnly.size());
    }
